...
1. duy trì alignment của bản thân;
2. tìm cách phá vỡ alignment của đối thủ;
3. chưa làm 2, nếu chưa đạt được 1.
...
kiểm soát đầu hoặc một chi sẽ đơn giản hơn rất nhiều nếu chúng ta xoay trục (rotation) một chút, và tốt nhất chính là để phương tạo lực của chúng ta vuông góc với phương chịu lực của đối thủ. phương pháp này gọi là kiểm soát bánh cóc (ratchet control). có 2 loại:
...
bạn chỉ nên duỗi chân ra khi việc đó thực sự giải quyết được tình trạng hiện tại, và không để cho đối thủ của bạn có cơ hội kiểm soát chi duỗi ra đó.

# 11. [elbow-knee connection](https://bjjmentalmodels.com/elbow-knee-connection/)

tương tự như trên, đây là một trường hợp cụ thể cho limb coiling. ta vừa có một cấu trúc tốt, vừa tránh việc bị kiểm soát đòn bẩy ở các chi, nhưng trên hết là ngăn đối phương kiểm soát phần hông. trong BJJ, và các môn wrestling/grappling nói chung, phần hông cực kỳ quan trọng.

ở vị trí dưới (bottom), khi đối thủ không thể tách được phần chân và tay của chúng ta ra, họ không thể pass guard được. ở trị trí trên (top), việc kết nối khuỷu tay - đầu gối cũng giúp tránh các hooks khó chịu từ đối thủ, tránh bị cô lập các chi.

# 12. [solid frames](https://bjjmentalmodels.com/solid-frames/)

một frame tốt cần có các tính chất sau:

1. dựa vào cấu trúc xương, không dựa vào cơ bắp;
2. chứa ít các khớp mà đối thủ có thể bẻ.

frame với cơ bắp chính là bạn đang tải (bench press) trọng lượng của đối thủ, cố nhiên sẽ gây mệt mỏi, hao thể thực. bạn frame bằng cách đẩy tay vào ngực đối thủ chính là tạo cơ hội cho họ armbar bạn.

hãy frame bằng *cẳng tay* thay vì bàn tay, bằng *ống đồng* thay vì bàn chân.

# 13. [kinetic chains](https://bjjmentalmodels.com/kinetic-chains/)

xuất phát là một khái niệm cơ học, nhưng được ứng dụng nhiều trong sinh lý học và huấn luyện thể thao, chuỗi động học chỉ sự phụ thuộc/bổ trợ lẫn nhau các khớp nối trong một liên kết động học. có hai dạng:

1. chuỗi mở: một đầu của chuỗi có thể di chuyển tự do, như chân/tay của bạn. dạng này không có nhiều ứng dụng trong jiu-jitsu;
2. chuỗi đóng: khi đầu tự do đó kết nối được tới một thứ không thể tự do di chuyển, ta có chuỗi đóng.

bằng cách kết nối như vậy, bạn có thể tạo ra frames tốt hơn, hoặc tạo ra khả năng sinh lực cao hơn. ví dụ cụ thể:

- móc hai mu bàn chân trong closed guard;
- kết nối hai tay khi siết guillotine.

# 14. [body tethering](https://bjjmentalmodels.com/body-tethering/)

việc trói buộc cơ thể của bạn vào phần core cơ thể của đối thủ, đặc biệt trong tính huống họ vẫn có base vững chắc và khả tăng tạo lực lớn, như:

- triangle từ bottom;
- hay back mount khi đối thử turtle.

đều mang đến rủi ro. bạn đã sử dụng hết các chi cho việc kiểm soát nhưng việc kiểm soát này nhiều khả năng sẽ trở thành giữ chính trọng lượng của bạn khỏi rơi. luôn cân nhắc các rủi ro và đừng giả định rằng trọng lượng của bạn là đủ để tạo áp lực kiểm soát.

# 15. [inside channel control](https://bjjmentalmodels.com/inside-channel-control/)

nếu như limb coiling hay elbow-knee connection giúp bạn phòng thủ tốt hơn thì inside channel control lại giúp bạn "khui" lớp phòng thủ đó của đối thủ ra.

kỹ thuật thường thấy nhất ở đây là "pummelling", đấu tay liên tục, thiết lập một hook từ "bên trong", từ đó đánh ra. tựu trung lại, nếu bạn dùng một phần cơ thể như 1 cái nêm (wedge) để tách rời các chi của đối thủ, đó chính là inside channel control. nó hiệu quả bởi vì:

1. tạo lever control, và chống lever control từ đối thủ;
2. cho bạn sự cơ động;
3. tạo lợi thế khi ở cự ly gần, rất khó cho đối thủ tạo không gian;
4. cho bạn khả năng kiểm soát phần core của đối phương.

# 16. [single vs. double lever control](https://bjjmentalmodels.com/single-vs-double-lever-control/)
