...
If we set the gain to `1`, the std is shrinking, and the saturations is coming to zeros, due to the first layer is pretty decent, but the next ones are shrinking to zero because of the `tanh()` - a squashing function.
...
But if we set the gain is far too high, let's say `3`, we can see the saturations is too high.
...
Similarly, we can do the samething with gradients. With the setting of gain as `5/3`, the distribution of gradients through layers quite the same.
...
Now imagine if we remove the `tanh` from all layers:
...
The recommend gain now for Linear is `1`. But no matter of how many Linear Layers you stacked up, you'll always get linear network, which will greatly limit the capacity of the neural nets.
