...
A sloping line also can be added using `geom_abline()` (later we will learn how to add a linear fit with `stat_smooth(method = "lm")`):
...
Or you can use `annotate(geom = "segment")` to add a sloping line:
...
If we set `geom = "curves"`, we have curved lines:
...
## just label 1% dataset, equally for each season
sample <- chic |>
...
ggplot(sample, aes(x = date, y = temp, color = season)) +
  geom_point(data = chic) +
  geom_label(aes(label = season), hjust = .5, vjust = -.5) +
  ## can use `geom_text()` if we dont want boxes around lables
  labs(x = "Year", y = "Temperature (°F)") +
...
Text are overlapping! the the package `{ggrepel}` comes to solve, just simply replace `geom_text()` by `geom_text_repel()`, and `geom_label()` by `geom_label_repel()`:
...
It may look nicer when we map `season` to `fill` instead of `color`:
...
Several ways: `annotate(geom = "text")`, `annotate(geom = "label")`, `geom_text()` or `geom_label()`:
...
The problem is ggplot had drawn one text for one data point ~ 1462 labels and you only see one. We can solve this by:
...
Change the properties of displayed text:
...
You might run into trouble when annotating with facets:
...
One workaround might be calculating the midpoint beforehand:
...
And there are also advanced solotions:
...
`{ggtext}` provide syntax for markdown:
...
They allow lots of customizations:
...
The other from `{ggtext}` is `geom_textbox()`:
...
ggplot(chic, aes(x = season, y = o3)) +
  ...
  # can achieve this with argument orientation = "y", without coord_flip()
...
Literally force axis ratio in Cartesian coordinate system:
...
ggplot(chic, aes(x = date, y = temp, color = o3)) +
  geom_point() +
  labs(x = "Year", y = "Temperature (°F)") +
  scale_y_reverse() # same for x
```

This will only work for continuous data. If you work with discrete variable you should use `fct_rec()` from `{forcats}` package.

## 15.4 Transform an axis

Instead of using linear mapping we can use `scale_y_log10()` or `scale_y_sqrt()`:

```{r}
ggplot(chic, aes(x = date, y = temp, color = o3)) +
  geom_point() +
  labs(x = "Year", y = "Temperature (°F)") +
  scale_y_log10(lim = c(0.1, 100)) # be careful of N/A
```

## 15.5 Circularize a plot

Circularize of polarize?:
...
Using this approach we can create piechart:
...
Be aware that there might be people who never seen a box/or whisker plot before:
...
Let's just show all the data points:

```{r}
g + geom_point()
```

Add transparency to make it more informative:

```{r}
g + geom_point(alpha = .1)
```

Not working so well, the overlapping is still too high!

## 16.1.2 Jitter the points

More readability, but be careful that you are adding noise to your data. Make sure audiences do not misinterprete it:
...
Why dont try some combinations:
...
We can do more, control the width of the jitter by the so called `geom_sina()` from `{ggforce}`:
...
The rug representation supplement to scatter or heatmap plot to visualize overall distribution:
